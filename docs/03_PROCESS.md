...
함수 실행 시, return address는 호출 주소가 아닌 호출 후 복귀할 주소를 기록한다. ARMv7 아키텍처에서, 명령어 주소는 4바이트씩 올라가므로, return address에는 호출 지점 +0x4 주소가 저장된다. ftrace가 콜스택을 생성할 때는 이 return address를 기준으로 함수명, 오프셋, 함수크기를 보여주게 되는데, 따라서 함수 오프셋이 0x0일 경우 해당 함수가 아닌, 해당 함수 시작 주소 전이 호출되었다고 보아야한다.

따라서 함수 오프셋이 0x0일 경우, 해당 함수가 실행된 것이 아닐 가능성이 크다. objdump를 사용해 확인해보면, pid_child_should_wake 전, __se_sys_exit_group에서 do_group_exit이 호출되었음을 확인할 수 있다.
